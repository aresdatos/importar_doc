...
export function parseHeaderData(text: string): HeaderData {
  ...
  // If patterns didn't find matches, use first non-empty line as name
  if (!header.name && lines.length > 0) {
    for (const line of lines) {
      if (line && !line.match(/(?:ruc|nit|rif|cuit|cnpj|rut|rnc|nif|cif|tax\s*id|fecha|date|factura|documento)/i)) {
        header.name = line;
        break;
      }
    }
  ...
  // Look for tax ID in all lines if not found
  if (!header.taxId) {
    for (const line of lines) {
      const match = findMatch(line, PATTERNS.taxId);
      if (match) {
        header.taxId = match;
        break;
      }
